...
function startGame(){console.log("Starting a game.");animationID=setInterval(function(){moveEverything();collision()&&(clearInterval(animationID),deathAnimation(),$("#timePlayed").append("<strong>Time:</strong> "+seconds+"."+1E3*ticks+" s"),$("#totalScore").append("<strong>Score:</strong> "+(balls.length*seconds+ticks/30).toString()),$("body").append('<div id="startGame">Restart</div>'),$("#startGame").unbind().click(function(){for(var a=0;a<ballElements.length;a++)ballElements[a].remove();player.xPos=
0;player.yPos=0;seconds=ticks=player.vel=0;balls=[];ballElements=[];$("#timePlayed").text("");$("#totalScore").text("");startGame();$("#startGame").remove()}));60===++ticks&&(ticks=0,0===++seconds%10?addBall():7<seconds%10?$("#originBox").css("background-color","red"):$("#originBox").css("background-color","white"))},17);addBall()}function addBall(){var a=new Ball(300,250),b=$('<div class="ball"></div>');balls.push(a);ballElements.push(b);$("#space").append(b)}
