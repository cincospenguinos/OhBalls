...
function calculateScore(){for(var a=0,b=seconds+ticks/60,c=0;c<balls.length;c++)var d=balls[c],a=a+(Math.abs(d.xVel)+Math.abs(d.yVel));c=b*balls.length*a;c-=15*(Math.pow(balls.length,2)-balls.length)/2;log.append("<tr><td>"+trials.toString()+"</td><td>"+balls.length.toString()+"</td><td>"+a+"</td><td>"+b+"</td><td>"+c.toString()+"</td></tr>");trials++;return c}
